...
public class ChuLiuEdmonds extends Algorithm {
    ...
    public void doStepA() {
        ...
        if ( it.hasNext() ) {
            ...
            System.out.println("  edge : " + this.edgeToString(e) + " w=" + e.getProperty("weight"));
    ...
    protected Edge getIncomingEdge(Vertex v) {
        return  (this.branching == Branching.MIN) ? this.getMinIncomingEdge(v) : this.getMaxIncomingEdge(v);
    }
    ...
    private void reconstructWorkingGraph(){
        ...
        this.cycleVertex.setProperty("cycleId", this.cycles.size());
        this.setVertexCoords(this.cycleVertex, this.cycleVertices.iterator().next());
        ...
        for (Edge e : this.workingGraph.getEdges()){
            ...
            if (this.cycleVertices.contains(e.getVertex(Direction.OUT))){
                ...
                newEdge = this.cycleVertex.addEdge(e.getLabel(), inV);
                ...
                newEdge = outV.addEdge(e.getLabel(), this.cycleVertex);

                Integer oldWeight = Integer.parseInt((String)e.getProperty("weight"));
                Integer cycleEdgeWeight = this.getCycleInnerEdgeWeight(e.getVertex(Direction.IN));

                System.out.print(this.edgeToString(newEdge) + " c=");
    ...
    public void doStepB() {
        ...
        this.cycleVertex = this.getCycleVertex();
        boolean outTreeRoot = this.isOutTreeRoot(this.cycleVertex);
        ...
        for (Edge e : this.edgeBucket){
            ...
            if ( e.getVertex(Direction.IN).equals(this.cycleVertex) ){
            ...
            else if ( e.getVertex(Direction.OUT).equals(this.cycleVertex)) {
                // (ui,x)
                Object weight = null;
                Vertex ov = null;
                Vertex tmpov = null;
        ...
        // process cycle edges
        Edge innerCycleExtremeEdge = this.branching == Branching.MIN ? this.getInnerCycleMaxEdge() : this.getInnerCycleMinEdge();
